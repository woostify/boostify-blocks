...
const WcbFontFamilyPicker = ({
  ...
  const selectedFontNames = (globalVars?.selectedFonts || "").split(",").map(f => f.trim()).filter(Boolean);
  const filteredGoogleFontsList = allowOnlySelected && selectedFontNames.length > 0 ? googleFontsList.filter(f => selectedFontNames.includes(f.value)) : googleFontsList;
  ...
  const FONTS = [{
    value: "",
    label: "Default"
  }, ...systemFonts, ...filteredGoogleFontsList];
