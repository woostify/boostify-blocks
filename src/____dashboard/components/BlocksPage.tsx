...
import { Bcb_blocks_enable_disable_options_Type, Bcb_block_Type } from "../App";
...
interface Props {
	initWcbBlocksEnableDisable: Bcb_blocks_enable_disable_options_Type;
	initWcbBlocksList: Bcb_block_Type[];
}
...
const BlocksPage: FC<Props> = ({
	...
	const handleDisableEnableBlocks = (obj: any) => {
		...
		const data = {
			action: "bcb_dashboard_blocks_disable_enable",
		...
		return (
			<div className="flex space-x-3 justify-end">
				<button
					...
					onClick={() => {
						const newblocksStatus: Bcb_blocks_enable_disable_options_Type =
							Object.keys(blocksStatus).reduce(
								(obj: Bcb_blocks_enable_disable_options_Type, item) => {
									return {
										...obj,
										[item]: "enabled",
									};
	...
	const renderCard3 = (block: Bcb_block_Type, index: number) => {
		if (block.parent) {
			return null;
		}
		...
		return (
			...
			>
				<div className="flex items-center gap-x-4 border-b border-gray-900/5 bg-gray-50 p-6 ">
					...
					<Menu
						as="a"
						href={`https://boostify-blocks.com/${name.replace(/\//g, "-")}`}
						target="_blank"
