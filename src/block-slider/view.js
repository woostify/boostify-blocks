...
const equalize = (wrap) => {
	...
	const items = wrap.querySelectorAll(".bcb-slider__item-inner");
...
const bootstrap = () => {
	const wraps = document.querySelectorAll(".bcb-slider__wrap");
	wraps.forEach((wrap) => {
		// Read attributes JSON saved in markup
		const pre = wrap.querySelector("pre[data-bcb-block-attrs]");
		let props = {};
		try {
		...
		if (window.jQuery) {
			const $wrapItems = window.jQuery(wrap).find(".bcb-slider__wrap-items");
